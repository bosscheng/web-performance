...
!(function () {
    ...
    var _promiseRejectList = []; // promise reject 错误。
        ...
        if (performance) {
            ...
            for (
                ...
                var resourceObj = {
                    ...
                    // 资源类型：['script','link','img','css','fetch','other','xmlhttprequest']
                    initiatorType: resourceItem.initiatorType,
                    // 资源名称
                    name: encode(resourceItem.name),
                    // 资源加载耗时
                    duration: resourceItem.duration || 0,
                    ...
                    // 资源所用协议
                    protocol: resourceItem.nextHopProtocol
                };
    ...
    function _addEvents() {
        ...
        // 监听promise reject 未捕获报错。
        addEventListener(window, 'unhandledrejection', function (event) {
            const error = formatPromiseError(event);
            _promiseRejectList.push(error);
        });
